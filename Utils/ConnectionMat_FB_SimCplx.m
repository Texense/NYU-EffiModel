...
%      PostUse PreUse: index arries, same size as N_Post/Pre
% Output:
...
function C_PostPre = ConnectionMat_FB_SimCplx(N_Post,NnExPost,Size_Post,  PostMap_Ori2Ext, PostUse,...
                                              N_Pre, NnExPre, Size_Pre,   PreMap_Ext2Ori,  PreUse,...
...
postNn_Loc = [NnExPost.X(PostMap_Ori2Ext(PostUse));
              NnExPost.Y(PostMap_Ori2Ext(PostUse))]*Size_Post;
preNn_Loc = [NnExPre.X(ismember(PreMap_Ext2Ori,  PreUse));
             NnExPre.Y(ismember(PreMap_Ext2Ori,  PreUse))]*Size_Pre;
PreMap_Ext2OriModi = PreMap_Ext2Ori(ismember(PreMap_Ext2Ori,  PreUse));
% record all effective connection index
% Ind1 for post, Ind2 for pre
Ind1 = zeros(1,length(PostUse)*1000); Ind2 = zeros(1,length(PostUse)*1000); 
tic
subInd = 0;

for postNn = 1:length(PostUse) % for each post synaptic cell, do
    ...
    Ind1(subInd+1:subInd+length(ConnInd2)) = PostUse(postNn)*ones(size(ConnInd2)); % Use original post Ind
    Ind2(subInd+1:subInd+length(ConnInd2)) = PreMap_Ext2OriModi(ConnInd2);
...
Ind1(Ind1 == 0) = [];
Ind2(Ind2 == 0) = [];
C_PostPre = sparse(Ind1,Ind2,ones(size(Ind1)), N_Post, N_Pre);

if SameCell == 1
    C_PostPre = C_PostPre-diag(diag(C_PostPre));
end
