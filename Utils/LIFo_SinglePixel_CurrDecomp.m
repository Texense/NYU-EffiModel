...
%% This is for analyzing current decomposition
% note: It would be costly to save all time trace of V and currents. I am
% doing patch means now.
% Zhuo-Cheng Xiao 05/025/2023
function [f_EnIOut,...
          L4ERcdMat,L4IRcdMat, ...
...
L4ERcdCyc = cell(TCyc,1); % decompose all CURRENTS!
L4IRcdCyc = cell(TCyc,1);
LGNRcdCyc = cell(TCyc,1);
L6RcdCyc = cell(TCyc,1);
ambRcdCyc = cell(TCyc,1);
...
for TInt = 1:TCyc
    ...
    % Compute induced conductances separately!:
    %LGN
    LGNampa = single([S_Elgn * lgnPix_Events(1:2,:);            S_Ilgn * lgnPix_Events(3,:)]);
    GLGN = ifft(fft(LGNampa',n) .* repmat(fft(KerAMPA',n),1,3))'; 
    %L6
    L6ampa = single([S_EL6  * L6Pix_Events(1:2,:) * rhoE_ampa; S_IL6 * L6Pix_Events(3,:) * rhoI_ampa]);
    L6nmda = single([S_EL6  * L6Pix_Events(1:2,:) * rhoE_nmda; S_IL6 * L6Pix_Events(3,:) * rhoE_nmda]);
    GL6 = ifft(fft(L6ampa',n) .* repmat(fft(KerAMPA',n),1,3))' + ...
          ifft(fft(L6nmda',n) .* repmat(fft(KerNMDA',n),1,3))'; 
    %L4E
    L4Eampa = single([S_EE * L4Pix_EventsE(1:2,:) * rhoE_ampa; S_IE * L4Pix_EventsE(3,:) * rhoI_ampa]);
    L4Enmda = single([S_EE * L4Pix_EventsE(1:2,:) * rhoE_nmda; S_IE * L4Pix_EventsE(3,:) * rhoI_nmda]);
    GL4E = ifft(fft(L4Eampa',n) .* repmat(fft(KerAMPA',n),1,3))' + ...
           ifft(fft(L4Enmda',n) .* repmat(fft(KerNMDA',n),1,3))';  
    %L4I
    GL4I = GI;
    %amb
    ambampa = single(S_amb  * AmbPix_Events);
    Gamb = ifft(fft(ambampa',n) .* repmat(fft(KerAMPA',n),1,3))'; 
    ...
    %VRcdCyc{TInt} = VRcd;
    SpRcdCyc{TInt} = SpLIF;
    L4ERcdCyc{TInt} = nansum(GL4E.*(Ve-VRcd),2)*dt; % decompose all conductances
    L4IRcdCyc{TInt} = nansum(GL4I.*(Vi-VRcd),2)*dt;
    LGNRcdCyc{TInt} = nansum(GLGN.*(Ve-VRcd),2)*dt;
    L6RcdCyc{TInt} =  nansum(GL6.*(Ve-VRcd),2)*dt;
    ambRcdCyc{TInt} = nansum(Gamb.*(Ve-VRcd),2)*dt;
end

%% Computing Frs and Currents
SpRcdMat = cell2mat(SpRcdCyc')';
f_EnIOut = SpRcdMat'*LGNFreq;

%VRcdMat = cell2mat(VRcdCyc')';
L4ERcdMat = (cell2mat(L4ERcdCyc')'*LGNFreq)';
L4IRcdMat = (cell2mat(L4IRcdCyc')'*LGNFreq)';
LGNRcdMat = (cell2mat(LGNRcdCyc')'*LGNFreq)';
L6RcdMat =  (cell2mat(L6RcdCyc')'*LGNFreq)';
ambRcdMat = (cell2mat(ambRcdCyc')'*LGNFreq)';
