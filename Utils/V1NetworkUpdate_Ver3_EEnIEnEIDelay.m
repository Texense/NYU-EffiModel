...
% Update 04/23/2021 E-to-E, I-to-E, E-to-I delays are added
...
% I-to-E has 1.Delay 
IPSS_E = EIDlyRcd(:,1);%EPSS = e to e post synaptic spike effect
% first respond to spikes
% gaba
GE_gaba_R_inter = GE_gaba_R + S_EI * IPSS_E;  
GE_gaba_D_inter = GE_gaba_D + S_EI * IPSS_E;
GI_gaba_R_inter = GI_gaba_R + S_II * (C_II * SpI);  
GI_gaba_D_inter = GI_gaba_D + S_II * (C_II * SpI);
% ampa
GE_ampa_R_inter = GE_ampa_R + S_Elgn * SpE_lgn + S_amb * SpE_amb + S_EE * EPSS_E * rhoE_ampa          + S_EL6 * SpE_L6 * rhoE_ampa;
GE_ampa_D_inter = GE_ampa_D + S_Elgn * SpE_lgn + S_amb * SpE_amb + S_EE * EPSS_E * rhoE_ampa          + S_EL6 * SpE_L6 * rhoE_ampa;
GI_ampa_R_inter = GI_ampa_R + S_Ilgn * SpI_lgn + S_amb * SpI_amb + S_IE * EPSS_I * rhoI_ampa  + S_IL6 * SpI_L6 * rhoI_ampa;
GI_ampa_D_inter = GI_ampa_D + S_Ilgn * SpI_lgn + S_amb * SpI_amb + S_IE * EPSS_I * rhoI_ampa  + S_IL6 * SpI_L6 * rhoI_ampa;
% nmda
GE_nmda_R_inter = GE_nmda_R + S_EE * EPSS_E * rhoE_nmda         + S_EL6 * SpE_L6 * rhoE_nmda;
GE_nmda_D_inter = GE_nmda_D + S_EE * EPSS_E * rhoE_nmda         + S_EL6 * SpE_L6 * rhoE_nmda;
GI_nmda_R_inter = GI_nmda_R + S_IE * EPSS_I * rhoI_nmda + S_IL6 * SpI_L6 * rhoI_nmda;
GI_nmda_D_inter = GI_nmda_D + S_IE * EPSS_I * rhoI_nmda + S_IL6 * SpI_L6 * rhoI_nmda;
...
oEIDlyRcd = sparse(size(EIDlyRcd,1),size(EIDlyRcd,2));
oEIDlyRcd(:,1:end-1) = EIDlyRcd(:,2:end); % first, move all kicks one-dt forward
oEIDlyRcd(:,end) = C_EI * oSpI;
end