...
% Ver 4:       L6 added
% Ver 4.1:     For the small network
%% Ver 4.2:   MF+V+C!!
%i.e.: We solve a linear equation from MF+V, then do LIF incorporated with gamma
% iteration
function [f_EnIOut,meanVs,loop,SteadyIndicate,FailureIndicate] = MF_V_C_Drive_SNW(C_EE,C_EI,C_IE,C_II,... %4
                                   ...
                                   gL_E,gL_I,Ve,Vi,... %4
                                   aE,aI,varargin) %2+x % if varagin non empty, we only export the last state
                                   %This line: We are GUESSing aE aI, and use simulation to determin bE bI 

if nargin > 34+4  % Specify: The number of loops I want, after stopping criteria met    
    AveLoop = varargin{2};
else 
    AveLoop = 100;
end

if nargin > 35+4 % Specify: Maximum nubmer of loops before stopping
        StopLoop = varargin{3};
else 
        StopLoop = AveLoop;
end

if nargin > 36+4 % Specify: stepsize h
    h_Step = varargin{4};
else
    h_Step = 1;        
end

if nargin > 37+4 % Specify: LIF simulation timef_pre
    LIFSimuT = varargin{5};
else
...
N_EE = mean(sum(C_EE,2)); N_EI = mean(sum(C_EI,2)); 
...
f_EnI0 = MFpVpC_ab(N_EE,N_EI,N_IE,N_II,...
                                   S_EE,S_EI,S_IE,S_II,p_EEFail,...
                                   lambda_E,S_Elgn,rE_amb,S_amb,...
                                   lambda_I,S_Ilgn,rI_amb,...
                                   S_EL6,S_IL6,rE_L6,rI_L6,...
...
if nargin > 33+4 && strcmpi(varargin(1),'Mean') % Specify: I don't need the trajectory but the final ones    
...
% NOTE!!: For high LGN and L6 input, can't make it by bernouli anymore!!
% Should follow the setup for network: preintroduce all input at first.
...
Sp_lgn = full(PoissonInputForNetwork(1,lambda,T,dt));
Sp_amb = full(PoissonInputForNetwork(1,r_amb,T,dt));
Sp_L6  = full(PoissonInputForNetwork(1,r_L6,T,dt));
Sp_EV1 = full(PoissonInputForNetwork(1,rE*full(N_E)*(1-p_Fail),T,dt));
Sp_IV1 = full(PoissonInputForNetwork(1,rI*full(N_I),T,dt));
